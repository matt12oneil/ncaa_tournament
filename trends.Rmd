...
tourney_teams <- lazy_dt(all_hist) %>%
...
s16 <- ggplot(s16_teams, aes(x = adj_o_rk, y = adj_d_rk)) + geom_point() + gghighlight((adj_o_rk + adj_d_rk > 100) | (adj_o_rk > 50 & adj_d_rk > 50)) + labs(title = 'Sweet 16 Teams by AdjO & AdjD Rank', subtitle = 'Offense + Defense > 100 & Both Outisde top-50') + coord_fixed(ratio = 1, xlim = NULL, ylim = NULL, expand = TRUE, clip = "on")
plot(s16)
```

``` {r sweet16_gt, echo=FALSE}
lazy_dt(s16_teams) %>%
  filter((adj_o_rk + adj_d_rk > 100) | (adj_o_rk > 50 & adj_d_rk > 50))%>%
  select(team, year, seed = ncaa_seed, Tempo = adj_t_rk, O = adj_o_rk, D = adj_d_rk, O_EFG = off_e_fg_pct_rk, O_TO = off_to_pct_rk, O_OR = off_or_pct_rk, O_FTR = off_ft_rate_rk, D_EFG = def_e_fg_pct_rk, D_TO = def_to_pct_rk, D_OR = def_or_pct_rk, D_FTR = def_ft_rate_rk) %>%
  as.data.frame() %>%
  gt() %>%
  tab_header(title = 'Sweet 16 History',subtitle = 'Offense + Defense > 100 & Both Outisde top-50')
```
...
lazy_dt(ff) %>%
  ...
  as.data.frame() %>%
  gt() %>%
  tab_header(title = 'Teams with a Chance to Make the Sweet 16', subtitle = 'Offense + Defense <= 100 & One Inside top-50')
```
