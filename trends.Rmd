...
champs <- ggplot(champions, aes(x = adj_o_rk, y = adj_d_rk)) + geom_point() + labs(title = 'Champions by AdjO & AdjD Rank', subtitle = 'Any Teams with an Offense or Defense outside Top-20 is highlighted') + gghighlight(adj_o_rk > 20 | adj_d_rk > 20)
...
final <- ggplot(final_teams, aes(x = adj_o_rk, y = adj_d_rk)) + geom_point() + gghighlight(adj_o_rk > 10 & adj_d_rk > 10) + labs(title = 'Final Teams by AdjO & AdjD Rank', subtitle = 'Any Teams with an Offense and Defense outside Top-10 is highlighted')
plot(final)
```
...
final_teams %>%
  filter((adj_o_rk > 10 & adj_d_rk > 10) & (adj_o_rk > 30 | adj_d_rk > 30)) %>%
  select(team, year, seed = ncaa_seed, Tempo = adj_t_rk, O = adj_o_rk, D = adj_d_rk, O_EFG = off_e_fg_pct_rk, O_TO = off_to_pct_rk, O_OR = off_or_pct_rk, O_FTR = off_ft_rate_rk, D_EFG = def_e_fg_pct_rk, D_TO = def_to_pct_rk, D_OR = def_or_pct_rk, D_FTR = def_ft_rate_rk) %>%
  gt()
```

``` {r final_possible, echo=FALSE}
ff %>%
  filter((adj_o_rk <= 10 | adj_d_rk <= 10) & (adj_o_rk <= 30 & adj_d_rk <= 30)) %>%
  select(team, Tempo = adj_t_rk, O = adj_o_rk, D = adj_d_rk, O_EFG = off_e_fg_pct_rk, O_TO = off_to_pct_rk, O_OR = off_or_pct_rk, O_FTR = off_ft_rate_rk, D_EFG = def_e_fg_pct_rk, D_TO = def_to_pct_rk, D_OR = def_or_pct_rk, D_FTR = def_ft_rate_rk) %>%
  gt() %>%
  tab_header(title = 'Possible Teams to make the National Championship', subtitle = 'Offense or Defense in top-10 and Both in top-30')
```
...
f4 <- ggplot(f4_teams, aes(x = adj_o_rk, y = adj_d_rk)) + geom_point() + gghighlight(adj_o_rk > 15 & adj_d_rk > 15) + labs(title = 'Final 4 Teams by AdjO & AdjD Rank', subtitle = 'Any Teams with an Offense and Defense outside Top-15 is highlighted')
plot(f4)
```

```{r final4_gt, echo =FALSE}
f4_teams %>%
  filter((adj_o_rk > 10 & adj_d_rk > 10) & (adj_o_rk > 40 | adj_d_rk > 40)) %>%
  select(team, year, seed = ncaa_seed, Tempo = adj_t_rk, O = adj_o_rk, D = adj_d_rk, O_EFG = off_e_fg_pct_rk, O_TO = off_to_pct_rk, O_OR = off_or_pct_rk, O_FTR = off_ft_rate_rk, D_EFG = def_e_fg_pct_rk, D_TO = def_to_pct_rk, D_OR = def_or_pct_rk, D_FTR = def_ft_rate_rk) %>%
  gt()
```
...
ff %>%
  filter((adj_o_rk <= 15 | adj_d_rk <= 15) & (adj_o_rk <= 40 & adj_d_rk <= 40)) %>%
  select(team, Tempo = adj_t_rk, O = adj_o_rk, D = adj_d_rk, O_EFG = off_e_fg_pct_rk, O_TO = off_to_pct_rk, O_OR = off_or_pct_rk, O_FTR = off_ft_rate_rk, D_EFG = def_e_fg_pct_rk, D_TO = def_to_pct_rk, D_OR = def_or_pct_rk, D_FTR = def_ft_rate_rk) %>%
  gt() %>%
  tab_header(title = 'Teams with a Chance to Make the Final 4',subtitle = 'With either a top-15 Offense or Defense and both within top-40')
```

```{r elite8_gg, echo=FALSE}
e8 <- ggplot(e8_teams, aes(x = adj_o_rk, y = adj_d_rk)) + geom_point() + gghighlight(adj_o_rk > 20 & adj_d_rk > 20) + labs(title = 'Elite 8 Teams by AdjO & AdjD Rank', subtitle = 'Any Teams with an Offense and Defense outside Top-20 is highlighted')
plot(e8)
```
...
ff %>%
  ...
  select(team, Tempo = adj_t_rk, O = adj_o_rk, D = adj_d_rk, O_EFG = off_e_fg_pct_rk, O_TO = off_to_pct_rk, O_OR = off_or_pct_rk, O_FTR = off_ft_rate_rk, D_EFG = def_e_fg_pct_rk, D_TO = def_to_pct_rk, D_OR = def_or_pct_rk, D_FTR = def_ft_rate_rk) %>%
  gt() %>%
  tab_header(title = 'Teams with a Chance to Make the Elite 8',subtitle = 'Teams with a Top-20 Offense or Defense and Both in the top-50')
```

```{r sweet16_gg, echo=FALSE}
s16 <- ggplot(s16_teams, aes(x = adj_o_rk, y = adj_d_rk)) + geom_point() + gghighlight(adj_o_rk > 40 & adj_d_rk > 40) + labs(title = 'Sweet 16 Teams by AdjO & AdjD Rank', subtitle = 'Any Teams with an Offense and Defense outside Top-40 is highlighted')
plot(s16)
```

``` {r sweet16_gt, echo=FALSE}
s16_teams %>%
  filter((adj_o_rk > 30 & adj_d_rk > 30) & (adj_o_rk > 65 | adj_d_rk > 65))%>%
  select(team, year, seed = ncaa_seed, Tempo = adj_t_rk, O = adj_o_rk, D = adj_d_rk, O_EFG = off_e_fg_pct_rk, O_TO = off_to_pct_rk, O_OR = off_or_pct_rk, O_FTR = off_ft_rate_rk, D_EFG = def_e_fg_pct_rk, D_TO = def_to_pct_rk, D_OR = def_or_pct_rk, D_FTR = def_ft_rate_rk) %>%
  gt()
```
...
ff %>%
  filter((adj_o_rk <= 40 | adj_d_rk <= 40) & (adj_o_rk <= 65 & adj_d_rk <= 65)) %>%
  select(team, year, seed = ncaa_seed, Tempo = adj_t_rk, O = adj_o_rk, D = adj_d_rk, O_EFG = off_e_fg_pct_rk, O_TO = off_to_pct_rk, O_OR = off_or_pct_rk, O_FTR = off_ft_rate_rk, D_EFG = def_e_fg_pct_rk, D_TO = def_to_pct_rk, D_OR = def_or_pct_rk, D_FTR = def_ft_rate_rk) %>%
  gt() %>%
  tab_header(title = 'Teams with a Chance to Make the Sweet 16', subtitle = 'Offense and Defense in the Top 40 and both in top-65')
```

``` {r s16_expected, echo=FALSE}
tourney_teams %>%
  #filter(adj_o_rk <= 20 & adj_d_rk <= 20) %>%
  filter((adj_o_rk <= 40 | adj_d_rk <= 40) & (adj_d_rk <= 50 & adj_o_rk <= 50)) %>%
